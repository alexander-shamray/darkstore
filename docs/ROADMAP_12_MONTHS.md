...
- **MVP — 6 мес Optimistic / 8 мес Realistic** (см. [`MVP_PLAN.md`](./MVP_PLAN.md)) — фокус на быстром запуске и первых реальных заказах.
- **После публичного запуска ещё 4–6 месяцев** — улучшение продукта, автоматизация, подготовка к росту.
- **Ключевой принцип:** Не усложнять архитектуру раньше времени.

> **Гант ниже отражает Optimistic-сценарий.** Realistic = +2 месяца к публичному запуску (см. таблицу «Ключевые вехи» ниже).

```mermaid
gantt
    title Dark Store — Roadmap 12 месяцев (Optimistic, Май 2026 — Апрель 2027)
    dateFormat  YYYY-MM
    axisFormat  %m/%Y
...
> **Источник правды по срокам MVP — [`MVP_PLAN.md` § Канонический график вех](./MVP_PLAN.md#канонический-график-вех-источник-правды).** Таблица ниже синхронизирована с ним: каждая MVP-веха имеет два срока — Optimistic (happy path) и Realistic (с буфером на партнёрские API, UX-итерации, HA-настройку).

| Веха | 🟢 Optimistic | 🟡 Realistic | Описание |
|------|---------------|--------------|---------|
| **Backend MVP готов (M3)** | конец августа 2026 (мес. 4) | середина октября 2026 (мес. 5.5) | 1С интеграция + Angular shell, E2E smoke зелёный |
| **Closed Beta (M4)** | конец сентября 2026 (мес. 5) | конец ноября 2026 (мес. 7) | 50+ активных пользователей |
| **Публичный запуск (M5)** | конец октября 2026 (мес. 6) | конец декабря 2026 (мес. 8) | Первый реальный оборот |
| **1000 заказов в месяц** | конец января 2027 (мес. 9) | конец марта 2027 (мес. 11) | Стабильная работа |
| **Готовность к масштабу** | конец апреля 2027 (мес. 12) | конец июня 2027 (мес. 14) | Продукт может выдержать рост |

**Итого MVP (M0 → публичный запуск):** 6 месяцев Optimistic / 8 месяцев Realistic.
